...
class Ontology():
    ...
    def convert_classes(self, vocabulary, num_inherit=1, cls_filter=[]):
        '''
        vocabulary: {mid: idx}
        according to the self.leaf_to_parent, perform num_inherit times of inheritance
        num_inherit: 1 means only one level of inheritance
        cls_filter: filter the classes, only keep the classes in cls_filter
        output:
        ...
        if len(cls_filter) > 0:
            mids = [mid for mid in mids if mid in cls_filter]
        for mid in mids:
            parent_ids = [mid]
            for i in range(num_inherit):
                new_parent_ids = []
                for parent_id in parent_ids:
                    if parent_id in self.leaf_to_parent:
        ...
        for i, parent_mid in enumerate(self.classes_mapping):
            if parent_mid not in self.mid_to_name:
                parent_name = 'unknown'
                print('Warning: parent mid {} not found in ontology, using "unknown" as name'.format(parent_mid))
            else:
                parent_name = self.mid_to_name[parent_mid]
            idx2name[i] = parent_name
            for mid in self.classes_mapping[parent_mid]:
                mid2idx[mid] = i
        return mid2idx, idx2name, len(self.classes_mapping)
...
class AudioSet_dataset(Dataset):
    def __init__(self, root='audioset', modality=[], split='eval', sr=16000, duration=10, frame_duration=0.1, label_level='clip',
                 num_inherit=1, vocabulary={}, cls_filter=[]):
        ...
        for row in label.itertuples():
            ...
            if label not in vocabulary:
                new_vocabulary[label] = len(vocabulary)

        if vocabulary == {}: # update vocabulary
            vocabulary = new_vocabulary
            
        self.vocabulary = vocabulary
        self.ontolog = Ontology(root)
        self.mid2idx, self.idx2name, self.num_classes = self.ontolog.convert_classes(vocabulary, num_inherit=num_inherit, cls_filter=cls_filter)
        ...
        for segment_id in labels:
            ...
            segment_labels = [x for x in labels[segment_id] if x[2] in self.mid2idx]
            if len(segment_labels) == 0:
                continue
            else:
                for start, end, label in segment_labels:
                    if label not in self.mid2idx:
                        continue
                    else:
                        cls_index = self.mid2idx[label]
                    clip_label[cls_index] = 1
                    start_frame = int(start/ self.frame_duration)
                    end_frame = int(end/ self.frame_duration)
                    frame_label[start_frame:end_frame, cls_index] = 1
    ...
    def filter_modal(self, modal):
        ...
        for i in range(self.__len__()):
            ...
            if 'text' in modal and not os.path.exists(text_file):
                continue
            keep_index.append(i)
        print('before filtering:', len(self.clip_labels), 'after filtering:', len(keep_index))
...
class FSD50K_dataset(Dataset):
    def __init__(self, root_dir, split='eval', label_level='clip', sr=16000, duration=10):
        ...
        self.vocabulary = self.ontolog.convert_classes(self.vocabulary, num_inherit=2)
