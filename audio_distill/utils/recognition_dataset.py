...
class AudioSet_dataset(Dataset):
    def __init__(self, root='audioset', modality=[], split='eval', sr=16000, duration=10, frame_duration=0.1, label_level='clip',
        ...
        if vocabulary == {}: # find the vocabulary from labels_set
            for i, label in enumerate(labels_set):
                vocabulary[label] = i
            
        self.vocabulary = vocabulary
        self.ontolog = Ontology(root)
        self.class_map = self.ontolog.convert_classes(vocabulary, num_inherit=num_inherit, cls_filter=cls_filter)
        self.num_classes = max(self.class_map.values()) + 1 # incase multiple mid may have the same index
        ...
        for segment_id in labels:
            ...
            segment_labels = [x for x in labels[segment_id] if x[2] in self.class_map]
            if len(segment_labels) == 0:
                ...
                for start, end, label in segment_labels:
                    if label not in self.class_map:
                        ...
                        cls_index = self.class_map[label]
